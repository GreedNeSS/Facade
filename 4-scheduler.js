...
class Logger {
	static color(level) {
		return Logger.COLOR[level] || Logger.COLOR.info;
	}
...
class Task extends EventEmitter {
	...
	run() {
		if (!this.activ || this.running) return false;
		this.running = true;
...
scheduler.task('name1', '2021-07-22T16:40Z', done => {
	setTimeout(() => {
		done(null, 'task successed')
	}, 1000);
});

scheduler.task('name2', '2021-07-22T16:40Z', done => {
	setTimeout(() => {
